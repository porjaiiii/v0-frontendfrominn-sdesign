...
//
// The rule is deliberately narrow: the caller proves who they are through
// getLineIdentity() (a LINE ID token or the session cookie), and may read
// only their own data. Staff who need to read someone else's profile go
// through /api/admin/profile/[id], which is a different claim (an admin
// session) rather than a widened version of this one.
...
/**
 * Verifies the caller through getLineIdentity() (a LINE ID token or the
 * session cookie it issued) and requires it to match `requestedId`.
 *
 * 401 and 403 are kept distinct on purpose: 401 means "you did not prove who
 * you are" (the client can retry once it has a token or a live session), 403
 * means "you did, and it is not your data" — which no retry fixes and which is
